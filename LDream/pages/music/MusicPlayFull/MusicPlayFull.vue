<template>
	<view class="musicFull h-full d-flex flex-column">
		<!-- <view class="bg" :style="{background: url(currentPlayMusic.al.picUrl)}">
		...
		<view class="main pl-4 pr-4 pt-3 pos-rel h-full">
			...
			<view class="w-full d-flex justify-center needleBox ">
				<u-image src="/static/needle.png" :class="['needle',{'playing':!!playing}]" width="28%"
					mode="widthFix"></u-image>
			</view>
			<view class="cd mt-4 ">
				<u-image :class="['avatar','musicActive',{musicStop:!playing}]"
					:src="currentPlayMusic.al.picUrl + '?param=400x400'"
					shape="circle" width="384upx" height="384upx"></u-image>
			</view>
		</view>
		<view class="util">
			<view class="progress w-full d-flex align-center pl-3 pr-3">
				<text>{{playTime}}</text>
				<canvas class="ml-1 mr-1" style="height: 20upx;" canvas-id="lineCanvas" id="lineCanvas" @touchmove="handleMove" @touchstart="handleTouch"></canvas>
				<text>{{musicTime}}</text>
			</view>
			<view class="d-flex w-full justify-between pl-5 pr-5 pt-4 align-center">
				...
				<view class="d-flex justify-between">
					...
					<view class="" @click="handleChangePlay">
						<u-image v-show="playing" class="ml-6 mr-6" src="/static/stopMusic.png" width="96upx" mode="widthFix"></u-image>
						<u-image v-show="!playing" class="ml-6 mr-6" src="/static/playMusic.png" width="96upx" mode="widthFix"></u-image>
					</view>
...
<script>
	...
	import {formatPlayTime} from "../../../util/time.js"
