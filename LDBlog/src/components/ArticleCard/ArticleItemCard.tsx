    ...
    <div className="itemArticleCard" onClick={handleClickToDetali}>
      <div className="title">
        <h2 className="p10">
          ...
          <div style={{ color: "#000" }}>{articleData.articleTitle}</div>
        </h2>
      </div>
      ...
      <div className="p10 time text-center">
        {moment(articleData.updated).format("YYYY-MM-DD")} by{" "}
        {articleData.userName} 发布于{" "}
        <span className="cate">
          {formatArticleName(articleData.articleCate)}
        </span>
      </div>
      <div className="content">{articleData.articleDesc}</div>
    </div>
  );
}
