            ...
            <div class="user-container">
                <div class="user-aside">
                    ...
                    <div :class="['playlist-main', slideBox1 ? 'active' : '']">
                        <router-link :to="{ name: 'userPlayList', query: { id: item.id } }" :class="['item', String(id) === String(item.id) ? 'active' : '']" class="item" v-for="(item, index) in createdList" :key="item.id">
                    ...
                    <div :class="['playlist-main', slideBox2 ? 'active' : '']">
                        <router-link :to="{ name: 'userPlayList', query: { id: item.id } }" :class="['item', String(id) === String(item.id) ? 'active' : '']" class="item" v-for="(item, index) in collectList" :key="item.id">
                            <el-image :src="item.coverImgUrl">
                                <div slot="placeholder" class="image-slot">
                                    <i class="iconfont icon-placeholder"></i>
                                </div>
...
export default {
    components: {},
    created () {
        // if (this.$route.path.indexOf('/user') >= 0 && !this.isLogin) {
    ...
    methods: {
        async getUserPlayList () {
            const { data: res } = await this.$http.playlistUser({ uid: this.currentUserInfo.userId, limit: '', offset: '' })
            if (res.code !== 200) {
                return this.$msg.error('数据请求失败')
            }
            console.log(`res`, res)
            this.createdList = res.playlist.filter(item => {
                return item.creator.userId == this.currentUserInfo.userId
            })
        ...
        async delPlayList (item, index, type) {
            ...
            this.$router.push({ path: '/user/playlist', query: { id: this.createdList[0].id } })
        },
        jump () {
